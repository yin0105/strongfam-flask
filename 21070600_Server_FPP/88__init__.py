...
app.config['MYSQL_DATABASE_PASSWORD'] = 'Uinm43xx'  
...
def findproject():
    ...
    crsr.execute("SELECT count(*) FROM idea WHERE projnum = %s and DATE_FORMAT(subdate,'%%Y-%%m-%%d') = %s and pilastname=%s and newprojnum is null",(projectnumber, submissiondate, pilastname)) 
...
def savemyidea():
    ...
    if form.validate_on_submit():
        ...
        crsr.execute("""
            INSERT INTO idea (projname,reqamount,newprojnum,orgname,orgwebaddr,schoolname,schoolwebaddr,pititle,picv,pifirstname,pimi,pilastname,pisuffix,piemail,pitele,piaddr1,piaddr2,picity,pistate,pizip,othertitle,otherfirstname,othermi,otherlastname,othersuffix,otheremail,othertele,otheraddr1,otheraddr2,othercity,otherstate,otherzip,irsletter_TF,budget_TF,confirmsent_TF,goal,description,aboutpeople,relevance,dissemination,projother) \
        ...
        crsr.execute("UPDATE idea SET newprojnum=%s WHERE projnum=%s", (projnum,existingprojnum))
        conn.commit()
...
def senddocument():
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            crsr.execute("UPDATE idea SET irsletter_TF=1, budget_TF=1 WHERE projnum=%s", (existingprojnum))
            conn.commit()
